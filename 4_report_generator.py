...
def get_inclusion_property(property):
    if property == 0:
...
L1_size = 1024  #int(sys.argv[2])
L1_assoc = 4    #int(sys.argv[3])
L2_size = 0 #int(sys.argv[4])
L2_assoc = 8    #int(sys.argv[5])
policy = 0  #sys.argv[6]
...
policies = [0, 1, 2]    #0->LRU    #1->FIFO    #2->OPTIMAL
cache_sizes = [2048, 4096, 8192, 16384, 32768, 65536]
inclusion_properties = [0, 1]
...
for property in inclusion_properties:
    X = []
    miss_Y = []
    AAT_Y = []
    for L2_size in cache_sizes:
        ...
        Hit_time_L2 = cache_access_data.get((L2_size, L2_assoc))
        ...
        sim_count += 1
        input_string = str(sim_count) + "=> " + "./sim_cache.exe 32 " +  str(L1_size) + " " + str(L1_assoc) + " " + str(L2_size) + " " + str(L2_assoc) + " " + str(policy) + " " + str(property) + " " + trace_file + " " + str(Hit_time_L1) + " " + str(Hit_time_L2) # + " " + str(miss_penalty)
        print(input_string)
        ...
        X.append(math.log(L2_size, 2))
        miss_Y.append(miss_rate)
        AAT_Y.append(AAT)
    
    value_dict_X[property] = copy.deepcopy(X)
    value_dict_miss[property] = copy.deepcopy(miss_Y)
    value_dict_AAT[property] = copy.deepcopy(AAT_Y)
...
# Plot each curve
for property in inclusion_properties:
    plt.plot(value_dict_X[property], value_dict_AAT[property], label="property = "+ get_inclusion_property(property))
